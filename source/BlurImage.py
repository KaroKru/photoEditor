...
class EnhTechniques:
    ...
    def histogram(self):
        ...
        distributionMap = [
            int(((distribution[i] - distribution[0]) /
                (histogramImage.width *
                histogramImage.height - distribution[0])) * 255)
        ...
        for i in range(histogramImage.width):
            for j in range(histogramImage.height):
                newPixels[i, j] = (
                    distributionMap[pixels[i, j][0]],
                    distributionMap[pixels[i, j][1]],
                    distributionMap[pixels[i, j][2]]
                )
    ...
    def averageHistogram(self):
        ...
        smoothSize = (3, 3)
        averageSmooth = [1 / 9] * 9
        smoothed = averageImage.filter(
            ImageFilter.Kernel(smoothSize, averageSmooth))
    ...
    def checkOptions(self):
        ...
        print("Possible image enhancement techniques:")

        for id, technique in enumerate(techniques, start=1):
            print(f"{id} -> {technique}")
